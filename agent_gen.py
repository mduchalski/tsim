...
def gen_agent(net, agents, agents_params):
    ...
    edge_weight = net[ agent['prev'][0], agent['next'][0] ]
    if on_edge.size:
        ...
        breaks = np.concatenate(([[0, 0]], breaks, [[edge_weight-min_dist, edge_weight-min_dist]]))
    else:
        breaks = np.array([[0, 0], [edge_weight, edge_weight]])
    x = random_disjoint(breaks)
    if x:
        agent['x'] = x
...
agents, agents_params, agents_routes = gen_agents(net, 1000)
fig, ax = plt.subplots()
net.plot(ax)
